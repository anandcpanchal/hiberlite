...
template<class C>
std::vector<sqlid_t> Database::getBeanIds(std::string condition_a)
{
	return dbSelectIds(con, getClassName<C>(), condition_a, "" );
}

template<class C>
std::vector< bean_ptr<C> > Database::getAllBeans(std::string condition_a)
{
	std::vector<sqlid_t> ids=getBeanIds<C>(condition_a);
	size_t N=ids.size();
